...
public class TitleItemDecortion extends RecyclerView.ItemDecoration {

	String TAG = "TitleItemDecortion";
	...
	public TitleItemDecortion(Context context,ArrayList<String> datas) {
		super();
		mDatas = datas;
		mPaint = new Paint();
		mBounds = new Rect();
...
/*
		...
		c.drawText(mDatas.get(position), child.getPaddingLeft()+60, child.getTop() - params.topMargin - (mTitleHeight / 2 - mBounds.height() / 2), mPaint);
	}
	...
	public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {//最后调用 绘制在最上层
		...
		c.drawText(tag, child.getPaddingLeft()+60,
